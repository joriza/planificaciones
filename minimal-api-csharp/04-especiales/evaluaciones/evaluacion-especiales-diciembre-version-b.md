# Evaluación del especial de diciembre — Versión B

> Dominio de esta versión: pacientes (tablas `patients` y `province_names` de `hospital.db`). Duración: 90 minutos. Resolución individual, sin celular, sin material consultable salvo lo provisto con esta prueba. Las condiciones completas y el criterio de resultado están en `evaluacion-especiales-diciembre.md`.

## Antes de empezar

- El trabajo va en el repositorio del grupo, en la carpeta `especial-diciembre/` (o en tu subcarpeta, si rinde más de un integrante del grupo).
...
git add .
git commit -m "especial-diciembre: primera version de la api"
...
| --- | --- | --- |
| 1 | Crear la mini API: dentro de la carpeta `especial-diciembre/`, creá el proyecto con los comandos de la hoja, instalá los paquetes, copiá `hospital.db` junto al `.csproj` y corré la API: debe quedar escuchando sin errores | 10 min |
