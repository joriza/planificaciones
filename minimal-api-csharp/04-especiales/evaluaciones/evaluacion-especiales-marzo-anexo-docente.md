# Anexo docente — Evaluación del especial de marzo (versiones A y B)

> Documento docente formal. No se entrega a los alumnos: contiene la solución completa de las versiones A y B, las respuestas esperadas, los criterios de logro por objetivo mínimo, los errores previstos y la pauta de registro y devolución. El archivo base es `evaluacion-especiales-marzo.md`.

## 1. Solución completa de la versión A (`Program.cs`)

```csharp
// Program.cs - Especial de marzo - Version A (solucion del docente)
...
```csharp
// Program.cs - Especial de marzo - Version B (solucion del docente)
...
| Unidad 4 — Flujo profesional del repositorio | Ítem 6 | Quedan el `README.md` breve en la carpeta del ejercicio, el issue «Especial de marzo», la rama `feature/marzo` con commits referentes, el pull request revisado y la fusión a `main` protegida |
...
- La verificación de la entrega por GitHub se realiza en los primeros minutos del plenario de cierre; el docente confirma la carpeta `especial-marzo/`, el issue, la rama, el pull request y el push de cada alumno presente.
