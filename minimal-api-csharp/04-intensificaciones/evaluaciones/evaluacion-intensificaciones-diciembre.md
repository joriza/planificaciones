# Evaluación de la intensificación de diciembre — Intensificación del camino mínimo

> Evaluación del momento de intensificación de diciembre (fuera de la planificación anual) · Curso: Minimal API con C# .NET 6. Este documento se entrega junto con la versión asignada (A o B). Leerlo completo antes de comenzar.

## 1. Identificación

| Campo | Detalle |
| --- | --- |
| Instancia | Intensificación de diciembre — intensificación del camino mínimo completo |
| Momento | Diciembre, finalizada la cursada regular; segundo encuentro del momento (fuera de la estructura anual de 36 encuentros) |
...
| 2 | GET con parámetro de ruta por id: 200 con el dato o 404 con mensaje | 15 min |
...
- **No apto aún:** al menos un objetivo queda sin lograr. El resultado habilita el plan de trabajo para marzo (`intensificaciones-marzo-intensificacion.md`), que evalúa el mismo camino mínimo con el mismo estándar.
...
- Distinto dominio: la versión A trabaja el dominio de médicos (tablas `doctors` y `admissions`); la versión B el dominio de pacientes (tablas `patients` y `province_names`).
- Ninguna regla que una tenga y la otra no: cambia la tabla de trabajo, no la exigencia; la validación de la escritura exige en cada versión los campos obligatorios que fija la tabla correspondiente, con la misma regla (campos obligatorios no vacíos).
...
- La constancia del ítem 6 va en el repositorio del grupo de cada estudiante; si rinde más de un integrante del mismo grupo, cada uno trabaja en su propia subcarpeta dentro de `intensificaciones-diciembre/`.
...
- Quienes obtengan No apto aún reciben por escrito el plan de marzo: qué objetivos pendientes tienen y qué guía de repaso usar; el estándar de marzo es idéntico al de diciembre.
