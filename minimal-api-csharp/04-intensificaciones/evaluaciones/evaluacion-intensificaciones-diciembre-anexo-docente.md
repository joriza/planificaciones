# Anexo docente — Evaluación de la intensificación de diciembre (versiones A y B)
...
- Verificar que cada estudiante puede acceder con sus credenciales al repositorio de su grupo (requisito del momento) y que la carpeta `intensificaciones-diciembre/` arranca limpia; si más de un integrante del mismo grupo rinde, acordar la subcarpeta de cada uno antes de empezar.
...
```csharp
// Program.cs - intensificaciones-diciembre - Version A (solucion del docente)
...
```csharp
// Program.cs - intensificaciones-diciembre - Version B (solucion del docente)
...
- A quien obtenga No apto aún se le entrega por escrito el plan de marzo: objetivos pendientes, guía de repaso y fecha; el estándar de marzo (`intensificaciones-marzo-intensificacion.md`) es idéntico al de diciembre, con más tiempo de preparación.
