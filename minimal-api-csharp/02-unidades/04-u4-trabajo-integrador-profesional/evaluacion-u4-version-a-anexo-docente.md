# Anexo docente — Evaluación de la Unidad 4 — Versión A

> Documento docente formal. No se entrega a los alumnos: contiene la solución completa de la versión A (estadística de médicos por especialidad), el recorrido esperado del flujo GitHub, las respuestas esperadas de los ítems conceptuales, los criterios de corrección ítem por ítem, la rúbrica de defensa individual con calificación y la pauta de devolución.

| Campo | Detalle |
| --- | --- |
| Instancia | Encuentro 32 — Evaluación de la Unidad 4 · prueba práctica breve, versión A |
| Dominio | Tabla `doctors`: estadística de médicos por especialidad (`GET /stats/doctors`) |
...
Inserción canónica sobre el `Program.cs` del grupo: el endpoint va **antes** de `app.Run()` y el record, **al final** del archivo. La versión A agrupa sobre la tabla `doctors` (una sola tabla, sin JOIN):

```csharp
// GET /stats/doctors: estadistica de medicos por especialidad (Issue #N)
app.MapGet("/stats/doctors", () =>
{
    // using: la conexion se cierra sola al salir del handler
    using var connection = new SqliteConnection(connectionString);

    // COUNT(*) cuenta los medicos de cada grupo; GROUP BY parte la tabla
    // por especialidad distinta. ORDER BY Doctors DESC ordena de mas a menos.
    // Cada columna lleva alias para encajar en el record DoctorCount
    var stats = connection.Query<DoctorCount>(
...
record DoctorCount(string Specialty, int Doctors);
```

Aceptaciones válidas menores: `ORDER BY COUNT(*) DESC` en lugar de ordenar por el alias (SQLite acepta ambas formas); `Results.Ok(connection.Query<DoctorCount>(...))` sin variable intermedia; alias de tabla en el `FROM` (`FROM doctors d` con `d.specialty`). **No se acepta** `TypedResults` (canon: `Results` en .NET 6) ni ordenar por `specialty` en lugar del conteo (la salida debe ir de más a menos).
...
- Pantalla: **Issues → New issue**. Título «Estadística de médicos por especialidad», etiqueta `estadistica`, descripción y los cinco criterios como lista de verificación (`- [ ]`).
...
git switch main
git pull
git switch -c feature/stats-doctors
git branch
```
...
```powershell
dotnet run
curl http://localhost:5080/stats/doctors
```

```powershell
git add .
git commit -m "trabajo-final: estadistica de medicos por especialidad (#N)"
git push -u origin feature/stats-doctors
```

Salida esperada del push: la rama se crea en GitHub y queda conectada (`Branch 'feature/stats-doctors' set up to track remote branch`). GitHub muestra el aviso **recent pushes** con el botón para abrir el pull request.

### Paso 3 — Pull request (GitHub web)

- Pantalla: **Pull requests → New pull request**. Verificar `base: main` ← `compare: feature/stats-doctors` (dirección correcta; invertida es defecto).
...
Salida esperada: `main` actualizada con el endpoint, que responde igual que en la rama. La rama local ya fusionada puede eliminarse (`git branch -d feature/stats-doctors`).
...
| 2 | Rama `feature/stats-doctors` creada desde `main` actualizada | 4 |
...
| Ítem | Qué se observa | Puntos |
| --- | --- | --- |
| 3 | PR con dirección correcta `base: main` ← `compare: feature/stats-doctors` | 5 |
...
| Ítem | Qué se observa | Puntos |
| --- | --- | --- |
| 4 | `GET /stats/doctors` responde `200` con la lista JSON (`Results.Ok`) | 6 |
| 4 | Cada fila tiene exactamente dos campos con alias correctos (`Specialty`, `Doctors`) | 5 |
| 4 | `COUNT(*)` con `GROUP BY specialty` | 6 |
| 4 | `ORDER BY Doctors DESC` (de más a menos) | 4 |
| 4 | Canon: `Results` (nunca `TypedResults`), record `DoctorCount` al final, comentarios en el código | 5 |
| 4 | Conexión con `using` dentro del handler; sin concatenar datos recibidos | 4 |
...
**Preguntas para la defensa:** del banco anticipado en el anexo docente del encuentro 31, dos o tres por integrante, mezclando un requisito que programó y uno del grupo; las transversales de flujo (rama de features, protección de `main`) conectan la defensa con la prueba práctica del día. Ante silencio, bajar la pregunta de nivel («¿qué línea de la consulta agrupa?»); ante un error no detectado, mostrar la salida y pedir la lectura. La corrección docente nunca reemplaza el intento del alumno.
...
| `500` al pedir el endpoint | Falta el `GROUP BY` o alias que no coincide con el record | Descuenta el ítem correspondiente; leer el error en la terminal de `dotnet run` es parte de la corrección |
| Salida ordenada por especialidad y no por cantidad | `ORDER BY specialty` | Descuenta el ítem de orden; el resto del endpoint puntúa |
...
- **Planilla:** grupo → versión A y puntaje por bloque de la prueba (15/20/25/30 + conceptuales); integrante → los ocho criterios de la rúbrica con su valor, nota de defensa, puntaje conceptual individual y composición final: 50 % defensa + 50 % prueba.
- Comentarios generales sugeridos: qué consolidó la unidad (flujo completo con trazabilidad, producto integrador) y los errores más vistos en la prueba (dirección del PR, orden del agregado, convención de commits).
- Los criterios no logrados se traducen en pistas concretas para los encuentros 34 y 35 (recuperación y profundización U3-U4): por ejemplo, reconstruir el flujo completo sobre un issue nuevo, o el molde de estadística con `GROUP BY`.
- La versión B es equivalente: misma estructura, mismo puntaje y mismas respuestas esperadas para los ítems conceptuales; el dominio cambia a `patients` + `province_names` (ver `evaluacion-u4-version-b-anexo-docente.md`).
