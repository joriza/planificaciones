# Evaluación de la Unidad 4 — Versión A

> Dominio de esta versión: estadística de **médicos por especialidad** (tabla `doctors` de `hospital.db`). Prueba práctica breve, complementaria de la defensa individual: se resuelve **por grupo**, sobre el repositorio del grupo (carpeta `trabajo-final/`). Duración: 90 minutos dentro del encuentro. Puntaje total: 100 puntos. Sin celular. Las condiciones completas están en `evaluacion-u4.md`.
...
```markdown
Título: Estadística de médicos por especialidad
Etiqueta: estadistica

## Qué se pide
Endpoint GET /stats/doctors que devuelva la cantidad de médicos por
especialidad, para completar el panel de estadísticas del hospital.

## Criterios de aceptación
- [ ] GET /stats/doctors responde 200 con la lista de estadísticas en JSON
- [ ] Cada fila tiene exactamente dos campos: especialidad y cantidad de médicos
- [ ] La consulta usa COUNT(*) con GROUP BY por especialidad y ordena por cantidad descendente
...
2. Crear la rama del feature desde `main` actualizada: `feature/stats-doctors`.
...
1. Abrir el PR: **Pull requests → New pull request**, verificando la dirección `base: main` ← `compare: feature/stats-doctors`.
...
| Campo | Detalle |
| --- | --- |
| Endpoint | `GET /stats/doctors` |
| Qué devuelve | `200` con la cantidad de médicos por especialidad, en JSON |
| Consulta | `COUNT(*)` con `GROUP BY` por especialidad, ordenada por cantidad descendente |
| Record | `DoctorCount` con dos campos: especialidad y cantidad (`int`, es conteo, no id) |
...
// - Consulta con COUNT(*) y GROUP BY por especialidad,
//   ordenada por cantidad descendente (ORDER BY ... DESC)
// - Columnas con alias (AS) para encajar en el record DoctorCount
...
```json
[{"specialty":"Internist","doctors":4},{"specialty":"Cardiologist","doctors":3},{"specialty":"General Surgeon","doctors":2}]
```

La forma de la salida es exactamente esa (lista JSON con pares especialidad/cantidad, de más a menos); los valores concretos dependen de la base del grupo. Después del merge, repetir la prueba desde `main` actualizada: el endpoint tiene que responder igual desde la rama fusionada.
