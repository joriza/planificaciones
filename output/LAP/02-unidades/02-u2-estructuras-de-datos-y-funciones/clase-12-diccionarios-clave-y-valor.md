...
5. Preguntá un segundo nombre y eliminálo del diccionario con `pop()` si existe, o mostrá un mensaje si no.
...
| Usar `del member["clave"]` sin verificar | Si la clave no existe → `KeyError` | Usar `pop("clave", None)` que es segura |