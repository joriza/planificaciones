...
| Uso de celular | Permitido solo para consultar documentación |
...
| Error | Causa | Fix |
| --- | --- | --- |
| `ValueError: invalid literal for int()` al ejecutar `int("diez")` | El usuario tipeó texto no numérico y `int()` no sabe convertirlo | Validar la entrada con `try/except ValueError` (lo vemos en el Encuentro 6) |
