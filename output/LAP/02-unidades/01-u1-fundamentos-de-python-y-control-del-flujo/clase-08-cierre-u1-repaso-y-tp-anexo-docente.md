...
|---|---|
| Crear repo en GitHub | Interfaz web, repositorio vacío `tp-lap` |
...
| ☐ | Repositorio en GitHub (`tp-lap`) visible | 1 |
...
| `git push` falla porque no hay remote configurado | No ejecutaron `git remote add origin <url>` | Guiar: `git remote add origin https://github.com/usuario/tp-lap.git` |
...
```powershell
powershell -File tools/generar-version-b.ps1 -Tabla input/materias/LAP/tabla-dominio.json -Origen output/LAP/02-unidades/01-u1-fundamentos-de-python-y-control-del-flujo/clase-08-cierre-u1-repaso-y-tp.md -Version B
```
