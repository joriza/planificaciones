...
**Consigna:** agregá una opción en el menú principal que permita al usuario **buscar un `película` por nombre y mostrar su información completa** (nombre, `moviePrice`, `stockCopies`). Si no existe, mostrar "Película no encontrado."

| Ítem | Consigna | Pts |
| --- | --- | --- |
| 1a | Agregar la opción en el menú principal del programa (ej. "5. Buscar película") | 10 |
| 1b | Implementar la función `find_product(name)` que recorre `movieList` y devuelve el película si existe, o `None` si no | 20 |
| 1c | Mostrar la información completa del película (nombre, tarifa, copias/copias disponibles) con f-strings | 15 |
...
| --- | --- | --- |
| Opción 5 | `5` | Pide nombre del película |
| Buscar película existente | `papa` | Muestra nombre, tarifa y copias |
| Buscar película inexistente | `xyz` | "Película no encontrado." |
...
```bash
git add .
git commit -m "evaluacion-u4: agregar busqueda de película"
