# Evaluación de la Unidad 4 — Versión A: gestión de notas (gestor de entregas)
...
|---|---|
| Versión | A |
| Dominio de datos | Gestión de notas: gestor de entregas de trabajos prácticos |
...
| Entrega | `trabajo-final/entregas.py` + `entregas.json` + sección del proyecto en el README; `main` al día con push |
| Aprobación | 60 puntos o más de 100 (rúbrica de la consigna maestra) y defensa individual apta |

## Consigna

Desarrollar el proyecto integrador del grupo: un gestor de entregas de trabajos prácticos, en `trabajo-final/entregas.py`, con persistencia en `entregas.json` (junto al `.py`, creado y leído por el propio programa). Cada registro es un diccionario con las claves `alumno` (texto), `trabajo` (texto), `fecha` (texto ISO `AAAA-MM-DD`) y `corregido` (booleano, `False` en el alta).

1. Ofrecer un menú con las opciones:
   - `1` Registrar una entrega: pedir alumno, trabajo y fecha, ninguno vacío (rechazar el campo vacío y volver a pedir); agregar el registro con `corregido` en `False` y guardar la colección completa.
   - `2` Listar entregas: listado numerado mostrando cada registro y su estado de corrección.
   - `3` Buscar por alumno: coincidencia sin distinguir mayúsculas de minúsculas.
   - `4` Registrar corrección: marcar como corregida la primera entrega sin corregir del alumno indicado y guardar el cambio.
   - `0` Salir: guardar la colección completa y terminar.
2. Cargar la colección al abrir desde `entregas.json`. Sin archivo: empezar con la colección vacía, sin error. Archivo dañado a mano: avisar y terminar con código de salida 1.
...
- Mínimo cinco issues del proyecto, definidos al iniciar la unidad y todos cerrados al entregar (por ejemplo: núcleo del menú, persistencia JSON, búsquedas y corrección, README, ajustes finales).
...
- Primera corrida sin `entregas.json`: arranque vacío sin error.
- `entregas.json` dañado a mano: aviso y salida con código 1.
- Ciclo completo: alta, listar, buscar, registrar corrección (verificar que el estado cambia a corregido) y re-corrida conservando los datos.
...
- [ ] `python entregas.py` corre desde su carpeta sin `traceback`.
- [ ] Menú con las cinco opciones y salida limpia.
- [ ] Registro con claves `alumno`, `trabajo`, `fecha` (ISO) y `corregido`.
- [ ] Persistencia en `entregas.json`: re-corrida conserva los datos.
