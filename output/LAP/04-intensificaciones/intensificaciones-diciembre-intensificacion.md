...
1. **(15 min)** Ejercicio 1 — programa base: crear un archivo `verduleria.py`. Declarar variables con los tipos correctos para representar un producto (\(producto\)) de la verdulería: nombre (str), precio (float, \(productPrice\)), kilos disponibles (float, \(stockWeight\)). Pedir datos con `input()` y convertir. Mostrar con f-string.
2. **(15 min)** Ejercicio 2 — condicionales: agregar validación. Si el producto tiene menos de 1 kilo (\(kilos\)) disponible, mostrar "Stock bajo". Si el precio supera los $500, aplicar un descuento del 10%. Usar `if`/`elif`/`else`.
...
1. **(15 min)** Ejercicio 4 — lista de productos (\(productList\)): almacenar productos como diccionarios dentro de una lista. Pedir datos por teclado y agregar con `append()`.
2. **(15 min)** Ejercicio 5 — mostrar productos con `for`: recorrer la lista de productos (\(productList\)) y mostrar cada uno con f-string formateado.
3. **(15 min)** Ejercicio 6 — buscar producto (\(productName\)) por nombre: recorrer la lista con `for` y comparar. Mostrar el producto encontrado o "Producto no encontrado".

### Cierre (20 min)

Verificación individual: el docente recorre los puestos y marca en la lista de verificación qué objetivos están logrados (1-6) y cuáles quedan pendientes para el encuentro siguiente. Cada estudiante hace commit del avance con mensaje `"feat: verduleria base y listado"`.
...
1. **(15 min)** Ejercicio 7 — funciones: refactorizar el programa anterior. Extraer cada operación a una función: `agregar_producto()`, `listar_productos()`, `buscar_producto()` (\(productName\)), `vender_producto()` (\(sellProduct\)).
...
3. **(15 min)** Ejercicio 9 — validación con `try`/`except`: envolver la conversión de precio (\(productPrice\)) y kilos (\(kilos\)) en un bloque `try`/`except` que capte `ValueError`. Mostrar "Dato inválido, intente de nuevo" y pedir el dato otra vez.

**Bloque 2 — Menú, entrega y defensa (45 min)**

1. **(15 min)** Ejercicio 10 — menú completo en memoria: opciones (1: agregar producto, 2: listar productos, 3: buscar producto, 4: vender producto, 5: factura (\(factura\)), 6: salir). Factura muestra el resumen de la venta actual.
