# Anexo docente — Evaluación de la intensificación de marzo — Versión A
...
El programa de referencia es idéntico al de diciembre (mismo dominio y mismo estándar). Se detalla a continuación:
...
La devolución se realiza al finalizar la defensa oral. Se devuelve con nota de Apto o No apto aún por objetivo mínimo. Si el objetivo mínimo queda pendiente, se informa la lista de objetivos pendientes y se notifica a dirección para la recomendación de recursar la materia si el período lo permite. Se registra la nota en la planilla de evaluación y se anota la letra de la versión asignada.

**Nota:** Esta instancia tiene el mismo estándar que diciembre. La diferencia es que el estudiante tuvo más tiempo de preparación (diciembre a marzo). Si el estudiante no alcanza el objetivo mínimo, se recomienda recursar la materia si el período lectivo lo permite.