...
def list_books(books, prices, copies):
    ...
    for i in range(len(books)):
        print(f"{books[i]}: ${prices[i]:.2f} — Ejemplares: {copies[i]}")
...
| Opción 1 — Registrar préstamo | `registerLoan` con validación de ejemplares y recargo del 10% | Si ejemplares < días → "Sin ejemplares disponibles". Si total > 200 → recargo aplicado | Ejecutar con ejemplares=2, días=5 y con total > 200 |
| Opción 2 — Mostrar libros | `for` sobre la lista de libros mostrando nombre, precio y ejemplares | Se muestran los 3 libros con sus datos | Ejecutar y verificar la salida |
...
- **Opción 1 — Registrar préstamo (20 pts):** 10 pts por la validación de ejemplares correcta. 10 pts por el recargo del 10% cuando el total supera $200.
- **Opción 2 — Mostrar libros (10 pts):** 5 pts por el `for` sobre la lista. 5 pts por la salida con f-string mostrando nombre, precio y ejemplares.
