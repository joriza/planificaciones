...
2. **(15 min)** Verificación 2 — condicionales: mostrar la validación de stock bajo y descuento por precio alto. Si no está implementado, se guía su creación en vivo.
...
1. **(15 min)** Verificación 4 — lista de productos: el estudiante muestra cómo agrega productos como diccionarios a la lista. El docente revisa que la estructura de datos sea correcta (diccionarios con claves consistentes).
...
3. **(15 min)** Verificación 6 — buscar producto por nombre: mostrar la función de búsqueda con resultados existentes y el mensaje "Producto no encontrado" para un nombre inexistente.
...
1. **(15 min)** Verificación 7 — funciones: el estudiante muestra las funciones `agregar_producto()`, `listar_productos()`, `buscar_producto()` y `vender_producto()`. El docente verifica que tengan parámetros y retorno donde corresponda.
...
3. **(15 min)** Verificación 9 — `try`/`except`: el estudiante ingresa un dato no numérico en precio o kilos y muestra que el programa captura el error sin romperse.

**Bloque 2 — Verificación de menú completo, entrega GitHub y defensa (45 min)**

1. **(15 min)** Verificación 10 — menú completo: opciones 1 a 6 funcionando (agregar, listar, buscar, vender, factura, salir). La factura muestra los totales de la venta.
...
El docente recorre los puestos con la lista de verificación. Cada estudiante explica brevemente: qué hace el programa, cómo valida los datos de entrada, la diferencia entre una lista y un diccionario en su programa, y una función que le haya costado implementar.
