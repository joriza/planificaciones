...
git checkout -b feature/get-patients
```

### 1.2 Resolver el issue

El alumno implementa el endpoint `GET /patients` en su repositorio local, hace commit y push a la rama `feature/get-patients`.

### 1.3 Abrir la PR

Desde GitHub, se abre una Pull Request de `feature/get-patients` a `main` con una descripción clara:

```
Implementación del endpoint GET /patients

- Agrega el record Patient con tipos canónicos (long para PatientId)
- Implementa GET /patients que retorna la lista completa de pacientes
- Implementa GET /patients/{patientId:long} con manejo de 404
...
Una vez aprobada la PR, se realiza el merge a `main`. La rama `feature/get-patients` se puede eliminar después del merge.
...
| --- | --- | --- | --- | --- |
| 1.1 Crear rama por feature | 10 | La rama `feature/get-patients` existe y parte de `main`. | Rama creada desde otra rama; nombre incorrecto. | Verificar la rama y su origen en `main`. |
| 1.2 Resolver el issue en la rama | 10 | El endpoint GET /patients está implementado en la rama de feature. | Endpoint no funcional; cambios en `main` directamente. | Verificar que los cambios están en la rama de feature y no en `main`. |
| 1.3 Abrir PR con descripción | 10 | La PR está abierta desde `feature/get-patients` a `main` con descripción clara. | PR sin descripción; dirección incorrecta de la PR. | Verificar la dirección y la descripción de la PR. |
