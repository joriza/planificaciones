...
3. **Cobertura en progreso — coherente, pero conviene hacerla visible.** Diseñados 36 encuentros; existen clases 4-8 (U1 completa con evaluaciones A/B) y 10-14 (U2 sin evaluación aún). Faltan: encuentros 1-3, evaluación U2, unidades 3-4, especiales, continuidad pedagógica, instancias 16/33/36, criterios de aprobación. Es estado WIP legítimo (los frenos lo explican), pero `verificar-curso.ps1` podría también *reportar cobertura*: qué encuentros del 1-36 tienen documento y cuáles no. Hoy el script valida forma, no avance.
...
- **Paso 1 ejecutado:** `git init` (rama `main`) + commit inicial con el estado completo del proyecto, incluido este documento.
- **CodeGraph:** verificado después del init de git. Requisito del índice: que el workspace sea la raíz de un repositorio Git real. Con el commit inicial cumplido, el índice quedó inicializado sobre esta raíz. Valor honesto para este proyecto: bajo (es 95% markdown; el índice aporta en proyectos con código fuente). Útil si `minimal-api-csharp/` incorpora los proyectos .NET de ejemplo.
