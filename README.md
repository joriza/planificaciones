...
> **Versión 1.0.0** — estable: sistema completo probado con dos materias (LSO, LAP), pipeline de generación optimizado, integración con GitHub Actions, y plan de mejoras completado. La estructura y herramientas están validadas y documentadas.

> Este repositorio es un **meta-proyecto de generación de material docente**: una biblioteca de reglas, un archivo de datos por materia y una suite de herramientas deterministas (`tools/`) que reemplazan autoría LLM por render. Implementación completa probada con materias [`output/LSO/`](output/LSO/README.md) (C#/.NET) y [`output/LAP/`](output/LAP/README.md) (Python).
> El plan de mejoras §10 está **completado con todos sus pasos**: P2-E (consistencia evaluaciones), P2-D (canonización U4), P3-B (fraseos naturales), P1-D (distribución por destinatario), P2-A (banco de frases). Sistemas de control de calidad: pre-commit hooks RDD, lint-canon, verify-curso, integración GitHub Actions, y pipeline de PDFs optimizado (-24.3% páginas).
...
| [`output/`](output/) | Una subcarpeta por curso (declarada en [Datos particulares] del prompt). Contiene el corpus derivado de la materia: CSV administrativos, clases y evaluaciones, el README índice — 100% regenerable — y los documentos de cierre del ciclo (07-cierre-anual: seguimiento, memoria e informes de mesa, generados una vez y luego propiedad del docente). La firma (`curso-data.json`) y la hoja de convenciones viven en `input/materias/<m>/`. Subdirectorios por destinatario: para docente, alumnos regulares, recuperación (intensificaciones/fortalecimientos) y administración (continuidad). |
| [`output/<curso>/pdf/`](output/) | PDFs consolidados por unidad/carpetas, con opción combinada, anexos docentes separados, numeración de código optimizada y plantilla de impresión (wkhtmltopdf, -24.3% páginas). |
| [`docs/`](docs/) | Documentación de proyecto: plan de mejoras completado, decisiones técnicas, métricas optimización y RFCs. |
| [`odd/`](odd/tasks/) | Evidencia de trabajo por feature (mapas maestros, instrumentación, registro de commits, work-unit commits RDD). |
| `.github/` | Workflows de GitHub Actions para validación pre-commit, lint automatizado y monitoreo de calidad. |
...
| `generar-cierre-anual.ps1` | Documentos de cierre del ciclo en output/<m>/07-cierre-anual/ (render determinista desde el curso-data + filas invariantes): planilla de seguimiento anual (CSV), memoria anual e informes de mesa diciembre/marzo. Protege los existentes sin -Force: una vez completados por el docente, no se regeneran. | `powershell -File tools\generar-cierre-anual.ps1 -Materia input\materias\<m> -Salida output\<curso>\07-cierre-anual` |
...
| 12 | output/<X>/07-cierre-anual/ — seguimiento, memoria anual e informes de mesa | **Fase 4** | Render determinista, cero LLM (generar-cierre-anual.ps1); solo si no existen |
| 13 | Puerta de salida | **Fase 4** | `verificar-curso.ps1` y `lint-canon.ps1` en verde |
...
**Nota**: El sistema incluye pre-commit hooks con Receipt-Driven Development (RDD) para validación automática de work-unit commits, configurado en `.git/hooks/pre-commit` y GitHub Actions. La calidad se garantiza con `lint-canon.ps1` (validación canon), `verificar-curso.ps1` (integridad del curso) y pipeline de PDFs optimizado.
...
- **Distribución por destinatario**: documentos generados para docente (con anexos), alumnos regulares (sin anexos), recuperación (intensificaciones/fortalecimientos) y administración (continuidad). Los PDFs consolidados se generan separadamente para evitar mezcla de material docente-alumno.
- **Calidad y optimización**: Pre-commit hooks RDD, GitHub Actions automatizados, PDFs optimizados (-24.3% páginas), números de código verde pálido+subrayado, y pipeline de generación probado con 500+ páginas por materia.
- Recuperación y alumnos sin regularidad: no se generan unidades pedagógicas ni de estudio autónomo nuevas para la recuperación — los momentos de intensificación (del ciclo, diciembre y marzo) reutilizan los contenidos mínimos y los materiales del curso (clases, TPs, evaluaciones A/B), y las guías de camino por punto de reingreso (plan de mejoras docs/plan-de-mejoras-del-proyecto.md §9.4, aprobado y pendiente de implementación) ordenan ese recorrido en forma determinista. Las unidades de estudio autónomo dedicadas solo revivirían ante un caso real documentado — p. ej. un alumno que necesite estudiar sin computadora o con un enfoque muy distinto del curso — y como nueva feature con decisión de canon.
