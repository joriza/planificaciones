...
| [`input/materias/`](input/materias/) | Una subcarpeta por materia (nombre a elección del docente = identificador): `<nombre>/materia.md` (ficha), `<nombre>/nota-catedra.md` (prosa libre, opcional) y `<nombre>/pedido.md` (datos particulares: libro, continuidad, memoria, frenos). |
...
2. **Limpieza:** borrar TODO el contenido de la carpeta del curso (p. ej. `output/LSO/`), incluida la hoja de convenciones técnicas regenerable, **pero conservando los archivos en `input/materias/`** (`curso-data.json`, `convenciones-tecnicas.md`, `materia.md`, `pedido.md` — son propiedad del docente y no se regeneran): todo lo demás se regenera en la corrida. No tocar `input/materias/`, `input/plantillas/`, `tools/`, `input/estructura-de-la-clase.md` ni el prompt plantilla.
3. **Fase 0:** el curso-data ya es del docente: validarlo (`tools\validar-curso-data.ps1 -Materia output\<nombre>`) y reutilizarlo tal cual; no se re-redacta ni se edita. La hoja de convenciones técnicas se recrea desde `input/plantillas/plantilla-convenciones-tecnicas.md` con **spike de verificación obligatorio** (borrada en la limpieza: la hoja nace de ejecutar, como en la corrida original); misma regla de propiedad.
